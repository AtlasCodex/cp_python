...
from pyemail import send_email
...
def run(name):
    ...
    #构造邮件内容
    #预测期号 + 预测结果
    subject = "【{}】预测期号：{}".format(name_path[name]["name"], int(current_number) + 1)
    body = "【{}】预测期号：{}，预测结果：{}".format(name_path[name]["name"], int(current_number) + 1, get_final_result(name, predict_features_))
